  ...
  return (
    ...
    >
      ...
      <div className="mx-auto max-w-3xl text-center">
        <Eyebrow tone="dark">Turnamen Nasional eFootball Mobile · Pilot Season</Eyebrow>
      </div>

      {/* ── THE VIDEO — hero centerpiece ── */}
      <div className="relative mx-auto mt-10 w-full max-w-3xl">
        <div className="absolute -inset-8 -z-10 rounded-[3rem] bg-brand-gold/20 blur-[90px]" aria-hidden />

        <div className="relative -skew-x-2 overflow-hidden rounded-[1.75rem] border border-brand-gold/25 bg-[#0a0a0f] shadow-brand-glow">
        ...
        {/* Floating stat badges pinned on the video frame (desktop) */}
        {FLOATING_STATS.map((s, i) => (
          <div
            key={s.label}
            className="absolute hidden -skew-x-6 rounded-2xl border border-brand-secondary/15 bg-white px-5 py-3.5 shadow-brand sm:block"
            style={{
          ...
          >
            <div className="flex skew-x-6 items-center gap-3">
              <s.icon className="h-5 w-5 text-brand-primary" />
              <div>
                <p className="font-jetbrains text-sm font-black text-brand-bg-dark">{s.value}</p>
                <p className="text-[10px] font-bold uppercase tracking-wide text-brand-bg-dark/45">{s.label}</p>
              </div>
            </div>
          </div>
        ))}
      ...
      {/* Mobile fallback for the pinned badges above (hidden on sm+) */}
      <div className="mx-auto mt-6 flex max-w-3xl flex-wrap justify-center gap-2.5 sm:hidden">
        {FLOATING_STATS.map((s) => (
          <span
            key={s.label}
            className="inline-flex items-center gap-1.5 rounded-full border border-white/10 bg-white/5 px-3.5 py-1.5 text-xs font-extrabold text-white/80"
          >
      ...
      <div className="mx-auto mt-12 max-w-2xl text-center">
        <h1 className="text-[2.5rem] font-black italic leading-[0.98] tracking-tighter text-white sm:text-6xl">
          THE PITCH
          <span className="mt-1 block bg-gradient-brand bg-clip-text text-transparent">IS YOURS.</span>
        </h1>
        ...
        <div className="mt-7 flex flex-wrap items-center justify-center gap-3">
          <span className="inline-flex items-center gap-1.5 rounded-full bg-white/5 px-3.5 py-1.5 text-xs font-extrabold text-white/70 ring-1 ring-white/10">
            <Wallet className="h-3.5 w-3.5 text-brand-gold" /> Rp 25.000 / Tim
          </span>
          <span className="inline-flex items-center gap-1.5 rounded-full bg-white/5 px-3.5 py-1.5 text-xs font-extrabold text-white/70 ring-1 ring-white/10">
            <Ticket className="h-3.5 w-3.5 text-brand-gold" /> Kuota 64 Slot
          </span>
          <span className="inline-flex items-center gap-1.5 rounded-full bg-white/5 px-3.5 py-1.5 text-xs font-extrabold text-white/70 ring-1 ring-white/10">
            <Users className="h-3.5 w-3.5 text-brand-gold" /> Maks. 2 Slot/Orang
          </span>
        </div>
        ...
        <div className="mt-8 flex flex-wrap items-center justify-center gap-x-6 gap-y-3 text-sm font-bold text-white/45">
...
function LiveStatusBar() {
  return (
    <section className="relative z-20 -mt-6 px-0">
      <div className="-skew-y-1 bg-brand-bg-surface py-4 shadow-md">
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mt-12 grid grid-cols-2 gap-3 sm:grid-cols-4">
          {REG_RULES.map((r) => (
            <div
              key={r.label}
              className="-skew-x-3 border border-brand-secondary/10 bg-white p-5 text-center transition-all hover:-translate-y-1 hover:border-brand-gold/40 hover:shadow-brand"
            >
        ...
        {/* Pricing / CTA card */}
        <div className="mt-16 relative -skew-y-1 overflow-hidden bg-gradient-brand px-8 py-10 shadow-brand-glow sm:px-14">
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mt-14 grid gap-7 sm:grid-cols-3">
          {USP_FEATURES.map((f) => (
            <div
              key={f.title}
              className="group relative -skew-x-3 border border-brand-secondary/10 bg-zinc-50 p-8 transition-all hover:-translate-y-1.5 hover:border-brand-gold/40 hover:shadow-brand-lg"
            >
              <div className="skew-x-3">
...
function ScheduleSection() {
  return (
    <section id="schedule" className="bg-zinc-50 px-5 py-24 sm:px-8">
      <div className="mx-auto max-w-4xl">
        ...
        <div className="mt-12 space-y-3">
          {SCHEDULE.map((item) => (
            <div
              key={item.title}
              className="flex flex-col gap-3 -skew-x-2 border border-brand-secondary/10 bg-white p-5 sm:flex-row sm:items-center sm:justify-between"
            >
...
function RewardSection() {
  return (
    <section id="reward" className="bg-brand-bg-light px-5 py-24 sm:px-8">
...
function FAQSection() {
  ...
  return (
    <section id="faq" className="bg-zinc-50 px-5 py-24 sm:px-8">
      <div className="mx-auto max-w-3xl">
        ...
        <div className="mt-10 space-y-3">
          {FAQS.map((f, i) => {
            ...
            return (
              <div key={f.q} className="overflow-hidden rounded-2xl border border-brand-secondary/10 bg-white">
                <button
...
function FinalCTASection({ onSignup }: { onSignup: () => void }) {
  return (
    <section className="bg-zinc-50 px-5 py-20 sm:px-8">
